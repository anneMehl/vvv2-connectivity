...
```{r}
p_crop <- crop(permeability, extent(permeability, 900, 5000, 500, 5000)) # (for Raster x, row 5 to 10, column 7 to 12) 
...
```{r}
sf <- load("output/homerange.rda")

p_crop <- crop(permeability, sf)
plot(p_crop)
...
p3 <- aggregate(permeability, fact = 100)

q1 <- aggregate(quality)
q2 <- aggregate(quality, fact = 100)
