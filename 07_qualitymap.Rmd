...
rspf_mod <- mod11_rspf_99boot_new
...
permeability <- raster("maps/permeability.tif")
permeability2 <- raster("maps/p233_s.asc")
p_courseTest <- raster("maps/permeability_coarse.tif")

quality <- raster("maps/quality.tif")
quality_2 <- raster("maps/q233_s.asc")
...
p_crop_migratory2 <- crop(permeability, e)
plot(p_crop_migratory2)

qu_crop_migratory2 <- crop(quality, e)
plot(qu_crop_migratory)


writeRaster(p_crop_migratory2, "maps/p_crop_migratory2.tif", overwrite = T)
writeRaster(qu_crop_migratory2, "maps/qu_crop_migratory2.tif", overwrite = T)
...
p460 <- aggregate(permeability, fact = 20)
p580 <- aggregate(permeability, fact = 25) 


p187_s <- aggregate(p_crop_migratory2, fact = 8)

q460 <- aggregate(quality, fact = 20) 
q580 <- aggregate(quality, fact = 25) 
q187_s <- aggregate(qu_crop_migratory2, fact = 8)
...
writeRaster(p187_s, "maps/p187_migratory2.asc", overwrite = T)
writeRaster(q187_s, "maps/q187_migratory2.asc", overwrite = T)

writeRaster(p233_s, "maps/p233_s.asc", overwrite = T)
writeRaster(q233_s, "maps/q233_s.asc", overwrite = T)
