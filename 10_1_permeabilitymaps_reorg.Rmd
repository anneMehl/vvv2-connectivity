...
random_points <- st_line_sample(roads, n = 2, density = 1/200, type = "random") %>% 
  st_as_sf()# one per 200 m


random_points2 <-  st_cast(random_points, 'POINT') %>% 
  sample_n(., 21) %>%
  mutate(points = 1) %>% 
  dplyr::select(points)
...
grid <- st_make_grid(random_points2, cellsize = 23.3588) %>% 
  st_as_sf() %>% 
  mutate(id = 1:nrow(.))

plot(grid)

crossings_green_sf <- st_as_sf(crossings_green_stars)
...
crossings_green_stars <- st_as_stars(crossings_green)
random_points_r <- st_rasterize(grid_int, crossings_green_stars)

plot(random_points_r)

plot(crossings_green)
plot(roads, add=T)
plot(random_points_r, add=T)



write_stars(random_points_r, "maps/random_points_rast.tif")
...
if (name_raster=="random_points"){
  crossings_green <- raster("maps/random_points_rast.tif")
...
plot(permeability_random_points)
str(permeability_random_points)
res(permeability_random_points)


writeRaster(permeability_random_points, "maps/permeability_random_points.tif", overwrite = T)
...
rast <- stack(quality6_0804, permeability_current, permeability_random_points)
...
writeRaster(rast[[3]], file="maps/permeability_random_points_crop_1711.asc", overwrite = T)
