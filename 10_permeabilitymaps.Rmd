...
values(rail_cross) <- 0
####### to remove the crossings, set dist_passage to a high value
dist_passage <- raster("maps/sspf/fence.tif")
values(dist_passage) <- 12570
}
...
# random_cross <- raster("maps/random_points.tif")
# writeRaster(random_cross, "maps/random_cross.tif", overwrite = T)
...
#### Make new raster map
```{r}
permeability_no_road_barrier <- prop_young_forest # just use any layer
values(permeability_no_road_barrier) <- predprob

plot(permeability_no_road_barrier)
str(permeability_no_road_barrier)
res(permeability_no_road_barrier)


writeRaster(permeability_no_road_barrier, "maps/permeability_no_road_barrier.tif", overwrite = F)
...
rast <- stack(quality6_0804, permeability8_with, permeability_no_road_barrier)
...
writeRaster(rast[[3]], file="maps/permeability_no_crossings_full_barrier_crop_0909.asc", overwrite = F)
