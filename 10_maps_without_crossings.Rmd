...
#### aggregate the layers to resolution I want to use * or did I need to do the log stuff first because if not the layers have different extents or resolution or something? *
```{r}
## Example
...
rail_cross <- aggregate(rail_cross, fact = 5, fun = max)
dist_building <- aggregate(dist_building, fact = 5, fun = mean)
...
road_cross <- aggregate(road_cross, fact = 5, fun = max)
crossings_grey <- aggregate(crossings_grey, fact = 5, fun = max)
crossings_high_use <- aggregate(crossings_high_use, fact = 5, fun = max)
crossings_low_use <- aggregate(crossings_low_use, fact = 5, fun = max)
crossings_green <- aggregate(crossings_green, fact = 5, fun = max)
dist_passage <- aggregate(dist_passage, fact = 5, fun = min)
dist_roads <- aggregate(dist_roads, fact = 5, fun = min)
road_traffic_high <- aggregate(road_traffic_high, fact = 5, fun = max)
...
newdat <- data.frame(intercept=1, step_length=116, squareLS = 116^2, newdat)
...
permeability2 <- slope # just use any layer
values(permeability2) <- predprob
...
```{r}
# Use this...
p_crop_stationary <- raster("maps/p_crop_stationary.tif")

# ... or this to crop
plot(permeability2)
e <- drawExtent()

crop_permeability2_stationary <- crop(permeability2, p_crop_stationary)

p_crop_permeability2 <- crop(permeability2, e)

plot(crop_permeability2_stationary)
str(crop_permeability2_stationary)
...
## write raster
```{r}
writeRaster(p_crop_permeability2, "maps/p_crop_permeability2.asc", overwrite = T)
```
